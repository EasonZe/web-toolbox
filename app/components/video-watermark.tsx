...
export default function VideoWatermark() {
  ...
  return <UtilityShell title="视频加水印工具" description="为视频添加文字或图片水印，支持单点、平铺与网格布局。">
    ...
    <div className="utility-columns">
      ...
      <div className="utility-panel utility-controls"><h2>水印设置</h2><fieldset disabled={busy} className="utility-controls">
        ...
        <label>水印布局</label><div className="watermark-layout-options">{([['single','单点水印'],['tile','平铺水印'],['grid','网格水印']] as const).map(([layout, label]) => <button key={layout} type="button" aria-pressed={settings.layout === layout} onClick={() => change({ layout })}>{label}</button>)}</div>
        <label className="utility-range-control"><span>大小 <output>{settings.size}%</output></span><input type="range" min={1} max={settings.type === "text" ? 20 : 80} value={settings.size} onChange={(e) => change({ size: +e.target.value })} /></label>
        <label className="utility-range-control"><span>透明度 <output>{settings.opacity}%</output></span><input aria-label="水印透明度" type="range" min={0} max={100} value={settings.opacity} onChange={(e) => change({ opacity: +e.target.value })} /></label>
        {settings.layout === "single" ? <><label>快捷位置</label><div className="watermark-positions">{[[5,5,"左上"],[50,5,"上中"],[95,5,"右上"],[5,50,"左中"],[50,50,"居中"],[95,50,"右中"],[5,95,"左下"],[50,95,"下中"],[95,95,"右下"]].map(([x,y,name]) => <button key={name} type="button" onClick={() => change({ x: Number(x), y: Number(y) })}>{name}</button>)}</div>
        <label className="utility-range-control"><span>水平位置 <output>{settings.x}%</output></span><input type="range" min={0} max={100} value={settings.x} onChange={(e) => change({ x: +e.target.value })} /></label>
        <label className="utility-range-control"><span>垂直位置 <output>{settings.y}%</output></span><input type="range" min={0} max={100} value={settings.y} onChange={(e) => change({ y: +e.target.value })} /></label></> : <label className="utility-range-control"><span>水印间距 <output>{settings.spacing}%</output></span><input type="range" min={4} max={40} value={settings.spacing} onChange={(e) => change({ spacing: +e.target.value })} /></label>}
