...
主站及三个视频解析 Worker 的源码与配置均已收录。抖音和快手 Worker 只读取各平台公开页面、官方播放器或认可的媒体 CDN，不再依赖第三方解析服务，因此 Fork 可以完整自托管。
...
| 快手 | `eason-kuaishou-api` | 直接使用快手公开分享页与认可的媒体 CDN | 源码与 Wrangler 配置已收录，可独立部署 |
...
- 主站与三个视频 Worker 均设置了明确的 `compatibility_date`、生产路由和日志可观测性。
